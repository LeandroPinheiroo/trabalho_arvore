...
int main(){
	...
	arvore->raiz = insere_na_arvore(arvore->raiz,10);
	insere_na_arvore(arvore->raiz,7);
	insere_na_arvore(arvore->raiz,18);
	insere_na_arvore(arvore->raiz,4);
	insere_na_arvore(arvore->raiz,9);
	insere_na_arvore(arvore->raiz,11);
	insere_na_arvore(arvore->raiz,25);
	insere_na_arvore(arvore->raiz,16);
	mostra_arvore(arvore->raiz);
	int h = altura(arvore->raiz);
	h = h-1;
	printf("Altura = %d\n", h);
	int fator = fator_balanceamento(arvore->raiz);
	printf("Fator = %d\n", fator);
	int grau = verifica_grau(arvore->raiz->esquerda->esquerda);
	printf("Grau = %d\n", grau);
	int nivi = verifica_nivel_no(arvore->raiz, 10);
	printf("Nivi = %d\n", nivi);
	return(0);
}